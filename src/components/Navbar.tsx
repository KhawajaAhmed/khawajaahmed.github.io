...
const Navbar = () => {
  ...
  return (
    <div className="fixed hidden md:flex justify-center z-50 top-2 sm:top-4 left-1/2 transform -translate-x-1/2">
      <motion.nav 
        className={`glass-effect rounded-full px-4 sm:px-6 py-2 sm:py-3 shadow-lg max-w-3xl transition-all duration-300 ${scrolled ? 'bg-background/90' : 'bg-background/70'} flex`}
      ...
      <div className="hidden">
        {isMenuOpen && (
        <motion.div 
          className="md:hidden absolute top-14 sm:top-16 left-1/2 transform -translate-x-1/2 glass-effect shadow-lg py-3 sm:py-4 px-4 sm:px-6 flex flex-col space-y-3 sm:space-y-4 rounded-xl w-[90%] max-w-xs text-center"
...
const MobileBottomNav = () => {
  ...
  return (
    <div className="md:hidden fixed bottom-0 left-0 right-0 z-50">
      <div className="glass-effect shadow-lg py-3 px-2 flex justify-around items-center relative">
        ...
        <Link href="#contact" className="flex flex-col items-center" onClick={(e) => handleNavClick(e, '#contact')}>
          <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
          </svg>
          <span className="text-xs mt-1">Contact</span>
        </Link>
        <div className="absolute -top-12 right-4 bg-background/90 dark:bg-background/90 p-2 rounded-full shadow-lg">
          <ThemeToggle />
        </div>
      </div>
    </div>
